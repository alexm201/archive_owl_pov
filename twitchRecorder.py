...
class TwitchRecorder:
    ...
    def __init__(self):
        ...
        self.oauth_token = ""
        # root path set to file directory. change if needed
    ...
    def loopcheck(self):
        while True:
            ...
            if status == 2:
                ...
                if not self.filename is '':
                    filename = self.filename + ".mp4"
                else:
                    filename = self.username + " - " + datetime.datetime.now().strftime(
                        "%Y-%m-%d %Hh%Mm%Ss") + " - " + (info['stream']).get("channel").get("status") + ".mp4"

                    # clean filename from unecessary characters
                    filename = "".join(x for x in filename if x.isalnum() or x in [" ", "-", "_", "."])

                recorded_filename = os.path.join(self.recorded_path, filename)
                print("Saving to " + recorded_filename)

                # start streamlink process
                subprocess.call(["streamlink","--twitch-oauth-token", self.oauth_token, "twitch.tv/" + self.username, self.quality,
                     "-o", recorded_filename])
...
def main(argv):
    ...
    usage_message = 'twitch-recorder.py -u <username> -q <quality> -n <filename> -p <path>'

    try:
        opts, args = getopt.getopt(argv, "hu:q:n:o:", ["username=", "quality=", "filename=", "path="])
